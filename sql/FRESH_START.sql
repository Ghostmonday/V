...
-- ===============================================
-- 9. ROW LEVEL SECURITY (RLS) & AUTH TRIGGERS
-- ===============================================

-- 9.1 AUTH SYNC TRIGGER (CRITICAL)
-- Syncs new users from auth.users to public.users
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.users (id, handle, display_name, role)
  VALUES (
    new.id,
...
-- 9.4 SERVICE ROLE ACCESS (OMNIPOTENT)
...
CREATE POLICY auth_update_own_profile ON users FOR UPDATE TO authenticated USING (id = public.current_uid()) WITH CHECK (id = public.current_uid());

-- ROOMS
-- Public rooms are visible to everyone
CREATE POLICY auth_select_public_rooms ON rooms FOR SELECT TO authenticated USING (is_public = true);
-- Private rooms visible to members
CREATE POLICY auth_select_private_rooms ON rooms FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM room_memberships rm WHERE rm.room_id = id AND rm.user_id = public.current_uid())
);
-- Users can create rooms
CREATE POLICY auth_insert_rooms ON rooms FOR INSERT TO authenticated WITH CHECK (created_by = public.current_uid());

-- MESSAGES
-- Users can view messages in rooms they are members of OR public rooms
CREATE POLICY auth_select_messages ON messages FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM room_memberships rm WHERE rm.room_id = room_id AND rm.user_id = public.current_uid())
    OR
    EXISTS (SELECT 1 FROM rooms r WHERE r.id = room_id AND r.is_public = true)
);
-- Users can insert messages in rooms they are members of
CREATE POLICY auth_insert_messages ON messages FOR INSERT TO authenticated WITH CHECK (
    sender_id = public.current_uid() AND
    EXISTS (SELECT 1 FROM room_memberships rm WHERE rm.room_id = room_id AND rm.user_id = public.current_uid())
);

-- ROOM MEMBERSHIPS
-- Users can view memberships
CREATE POLICY auth_select_memberships ON room_memberships FOR SELECT TO authenticated USING (
    user_id = public.current_uid() OR
    EXISTS (SELECT 1 FROM rooms r WHERE r.id = room_id AND r.is_public = true)
);
-- Users can join public rooms
CREATE POLICY auth_join_public_rooms ON room_memberships FOR INSERT TO authenticated WITH CHECK (
    user_id = public.current_uid() AND
    EXISTS (SELECT 1 FROM rooms r WHERE r.id = room_id AND r.is_public = true)
);

-- 9.6 FEATURE POLICIES (FROM ADD_FEATURE_RLS_POLICIES.sql)

-- THREADS
CREATE POLICY auth_select_threads ON threads FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM room_memberships rm WHERE rm.room_id = threads.room_id AND rm.user_id = public.current_uid())
);
CREATE POLICY auth_insert_threads ON threads FOR INSERT TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM room_memberships rm WHERE rm.room_id = threads.room_id AND rm.user_id = public.current_uid())
);

-- EDIT HISTORY
CREATE POLICY auth_select_edit_history ON edit_history FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM messages m JOIN room_memberships rm ON rm.room_id = m.room_id WHERE m.id = edit_history.message_id AND rm.user_id = public.current_uid())
);

-- BOTS
CREATE POLICY auth_all_own_bots ON bots FOR ALL TO authenticated USING (created_by = public.current_uid()) WITH CHECK (created_by = public.current_uid());
CREATE POLICY auth_select_active_bots ON bots FOR SELECT TO authenticated USING (is_active = true);

-- BOT ENDPOINTS
CREATE POLICY auth_all_own_bot_endpoints ON bot_endpoints FOR ALL TO authenticated USING (
    EXISTS (SELECT 1 FROM bots b WHERE b.id = bot_endpoints.bot_id AND b.created_by = public.current_uid())
) WITH CHECK (
    EXISTS (SELECT 1 FROM bots b WHERE b.id = bot_endpoints.bot_id AND b.created_by = public.current_uid())
);

-- ASSISTANTS
CREATE POLICY auth_all_own_assistants ON assistants FOR ALL TO authenticated USING (owner_id = public.current_uid()) WITH CHECK (owner_id = public.current_uid());

-- SUBSCRIPTIONS
CREATE POLICY auth_all_own_subscriptions ON subscriptions FOR ALL TO authenticated USING (user_id = public.current_uid()) WITH CHECK (user_id = public.current_uid());

-- CONVERSATIONS (VIBES)
CREATE POLICY auth_select_conversations ON conversations FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM conversation_participants cp WHERE cp.conversation_id = conversations.id AND cp.user_id = public.current_uid())
);
CREATE POLICY auth_insert_conversations ON conversations FOR INSERT TO authenticated WITH CHECK (created_by = public.current_uid());

-- CONVERSATION PARTICIPANTS
CREATE POLICY auth_select_participants ON conversation_participants FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM conversation_participants cp WHERE cp.conversation_id = conversation_participants.conversation_id AND cp.user_id = public.current_uid())
);

-- CARDS
CREATE POLICY auth_select_cards ON cards FOR SELECT TO authenticated USING (true);

-- CARD OWNERSHIPS
CREATE POLICY auth_select_own_cards ON card_ownerships FOR SELECT TO authenticated USING (owner_id = public.current_uid());

-- PRESENCE LOGS
CREATE POLICY auth_select_presence ON presence_logs FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM room_memberships rm WHERE rm.room_id = presence_logs.room_id AND rm.user_id = public.current_uid())
);
CREATE POLICY auth_insert_own_presence ON presence_logs FOR INSERT TO authenticated WITH CHECK (user_id = public.current_uid());

-- EMBEDDINGS
CREATE POLICY auth_select_embeddings ON embeddings FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM messages m JOIN room_memberships rm ON rm.room_id = m.room_id WHERE m.id = embeddings.message_id AND rm.user_id = public.current_uid())
);
