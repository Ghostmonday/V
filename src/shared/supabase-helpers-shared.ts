 ...
 */
export async function transaction<T = unknown>(operations: Array<() => Promise<T>>): Promise<T[]> {
  try {
    ...
    try {
      ...
      logError(
        'Transaction failed, operations may be partially applied',
 ...
 */
export async function executeTransaction(sqlStatements: string[]): Promise<any> {
  try {
