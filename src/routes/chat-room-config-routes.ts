 ...
 */
router.post('/:id/moderation-thresholds', authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    const roomId = req.params.id;
    const userId = req.user?.userId;
    ...
    if (warn_threshold !== undefined && block_threshold !== undefined && block_threshold < warn_threshold) {
    ...
    if (!isOwner && !isAdmin) {
      return res.status(403).json({ error: 'Permission denied - only room owner or admin can set thresholds' });
    }
