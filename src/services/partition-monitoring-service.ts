...
export async function recordPartitionQuery(
  ...
  try {
    ...
    const metrics: Partial<PartitionMetrics> = existing ? JSON.parse(existing) : {
      partitionMonth,
      queryCount: 0,
      avgQueryLatencyMs: 0,
      maxQueryLatencyMs: 0,
    ...
    logError('Failed to record partition query', error instanceof Error ? error : new Error(String(error)));
  }
}
...
export async function updatePartitionSize(
  ...
  try {
    ...
    const maxSizeGB = parseFloat(process.env.MAX_PARTITION_SIZE_GB || String(DEFAULT_MAX_PARTITION_SIZE_GB));
    ...
    const maxRows = parseInt(process.env.MAX_PARTITION_ROWS || String(DEFAULT_MAX_PARTITION_ROWS), 10);
    if (rowCount > maxRows) {
      await addPartitionAlert(
        partitionMonth,
    ...
    logError('Failed to update partition size', error instanceof Error ? error : new Error(String(error)));
  }
}
...
async function addPartitionAlert(partitionMonth: string, message: string): Promise<void> {
  try {
    ...
    logError('Failed to add partition alert', error instanceof Error ? error : new Error(String(error)));
  }
}
...
export async function getAllPartitionMetrics(): Promise<PartitionMetrics[]> {
  try {
    ...
    logError('Failed to get partition metrics', error instanceof Error ? error : new Error(String(error)));
    return [];
  }
}
...
export async function getPartitionHealthSummary(): Promise<{
  ...
  try {
    ...
    logError('Failed to get partition health summary', error instanceof Error ? error : new Error(String(error)));
    return {
      totalPartitions: 0,
      healthyPartitions: 0,
...
export async function calculateDynamicThresholds(): Promise<{
  ...
  try {
    ...
    const shouldCreatePartition = metrics.some(m => 
      m.sizeGB > maxPartitionSizeGB * 0.8 || 
      m.rowCount > DEFAULT_MAX_PARTITION_ROWS * 0.8
    );
    ...
    logError('Failed to calculate dynamic thresholds', error instanceof Error ? error : new Error(String(error)));
    return {
      maxPartitionSizeGB: DEFAULT_MAX_PARTITION_SIZE_GB,
      maxPartitionRows: DEFAULT_MAX_PARTITION_ROWS,
      queryLatencyThresholdMs: DEFAULT_QUERY_LATENCY_THRESHOLD_MS,
...
export async function clearPartitionAlerts(partitionMonth?: string): Promise<void> {
  try {
    ...
    logError('Failed to clear partition alerts', error instanceof Error ? error : new Error(String(error)));
  }
}
