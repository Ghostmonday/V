...
export async function sendMessageToRoom(data: {
  ...
  try {
    ...
    const roomIdValue =
      typeof validatedInput.roomId === 'string'
    ...
    await redis.publish(
      // Silent fail: if Redis down, message saved but not broadcast
      `room:${data.roomId}`,
      JSON.stringify({
        ...data,
...
export async function getRoomMessages(
  roomId: string | number | (string | number)[],
  since?: string
): Promise<any[]> {
  try {
