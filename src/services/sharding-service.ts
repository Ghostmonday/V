...
async function getShardRegistry(): Promise<Record<string, ShardConfig>> {
  try {
    ...
    // Cache in Redis
    await redis.set(SHARD_REGISTRY_KEY, JSON.stringify(registry), 'EX', 3600);

    return registry;
  } catch (error) {
    logError(
      'Failed to get shard registry',
...
export async function recordShardHealth(
  ...
  try {
    ...
    await redis.set(healthKey, JSON.stringify(healthData), 'EX', 3600);
 ...
 */
export async function executeOnShard<T>(shardId: string, queryFn: () => Promise<T>): Promise<T> {
