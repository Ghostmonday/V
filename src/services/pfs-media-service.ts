 ...
 */
export function deriveSharedSecret(ourPrivateKey: string, theirPublicKey: string): string {
  try {
 ...
 */
export async function getCallPublicKeys(
  callId: string
): Promise<Record<string, { publicKey: string; keyId: string }> | null> {
  try {
    const sessionKey = `pfs:call:${callId}`;
 ...
 */
export async function getUserPrivateKey(callId: string, userId: string): Promise<string | null> {
  try {
    const privateKeyKey = `pfs:call:${callId}:user:${userId}:private`;
...
export async function encryptMediaStream(
  ...
  return await mediaStreamCircuitBreaker.execute(
    async () => {
      try {
    ...
    async () => {
      ...
      logError(
        'Media stream encryption failed - circuit breaker fallback',
...
export async function decryptMediaStream(
  ...
  return await mediaStreamCircuitBreaker.execute(
    ...
    async () => {
      // Fallback: return error response instead of crashing
      logError(
        'Media stream decryption failed - circuit breaker fallback',
        new Error('Decryption service unavailable')
      );
