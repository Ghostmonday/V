 ...
 */
export async function isMessageEligibleForArchive(messageId: string): Promise<{ eligible: boolean; ageDays: number; reason?: string }> {
  try {
    ...
    return {
      ...
      reason: eligible ? undefined : `Message is only ${Math.floor(ageDays)} days old (requires ${ARCHIVE_AGE_DAYS} days)`,
    };
  } catch (error: any) {
 ...
 */
async function createArchiveFormat(message: any): Promise<{ format: string; data: string; checksum: string }> {
  try {
 ...
 */
export async function archiveMessage(messageId: string): Promise<{ success: boolean; archiveId?: string; error?: string }> {
  try {
    ...
    const validatedArchive = validateBeforeDB(archiveRecord, archiveRecordSchema.extend({
...
export async function retrieveArchivedMessage(messageId: string): Promise<any | null> {
  try {
    ...
    const calculatedChecksum = crypto.createHash('sha256').update(archive.archive_data).digest('hex');
    
    // VALIDATION CHECKPOINT: Validate archive checksum
    if (calculatedChecksum !== archive.archive_checksum) {
      logError('Archive integrity check failed', new Error(`Expected ${archive.archive_checksum}, got ${calculatedChecksum}`));
...
export async function archiveMessagesBatch(): Promise<{ archived: number; failed: number }> {
  try {
    ...
    if (archiveThreshold > new Date()) {
      logWarning('Archive threshold is in the future', { threshold: archiveThreshold.toISOString() });
    }
