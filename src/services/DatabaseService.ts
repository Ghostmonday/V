...
export function subscribeToChannel(
  ...
  try {
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          logError(`Failed to subscribe to channel: ${channelName}`, new Error('Channel subscription error'));
        }
      });
    ...
    logError(`Error subscribing to channel ${channelName}`, error instanceof Error ? error : new Error(String(error)));
    return null;
  }
}
...
export async function configureTTLPolicy(config: TTLConfig): Promise<boolean> {
  try {
    ...
    logError(`Error configuring TTL policy for ${config.tableName}`, error instanceof Error ? error : new Error(String(error)));
    return false;
  }
}
