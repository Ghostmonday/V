...
async function initSignalProtocol(): Promise<any> {
  ...
  libsignalPromise = (async () => {
    try {
      ...
      logError('Failed to load Signal Protocol library', error instanceof Error ? error : new Error(String(error)));
...
export async function generateIdentityKeyPair(): Promise<{
  ...
  try {
    ...
    logError('Failed to generate identity key pair', error instanceof Error ? error : new Error(String(error)));
...
export async function generatePreKeyBundle(identityKeyPair: {
  ...
  try {
    ...
    const identityPublicKey = signal.IdentityKey.deserialize(Buffer.from(identityKeyPair.publicKey));
    const identityPrivateKey = signal.PrivateKey.deserialize(Buffer.from(identityKeyPair.privateKey));
    ...
    logError('Failed to generate prekey bundle', error instanceof Error ? error : new Error(String(error)));
...
export async function encryptMessage(
  ...
  try {
    ...
    const senderIdentityPublic = signal.IdentityKey.deserialize(Buffer.from(senderIdentityKeyPair.publicKey));
    const senderIdentityPrivate = signal.PrivateKey.deserialize(Buffer.from(senderIdentityKeyPair.privateKey));
    const senderIdentityKeyPairObj = new signal.IdentityKeyPair(senderIdentityPublic, senderIdentityPrivate);
    
    // Reconstruct recipient identity key
    const recipientIdentityKey = signal.IdentityKey.deserialize(Buffer.from(recipientPreKeyBundle.identityKey));
    ...
    const signedPreKeyPublic = signal.PublicKey.deserialize(Buffer.from(recipientPreKeyBundle.signedPreKey.publicKey));
    const signedPreKeySignature = Buffer.from(recipientPreKeyBundle.signedPreKey.signature);
    
    // Verify signature
    if (!recipientIdentityKey.getPublicKey().verifySignature(signedPreKeyPublic.serialize(), signedPreKeySignature)) {
...
export async function decryptMessage(
  ...
  try {
    ...
    const recipientIdentityPublic = signal.IdentityKey.deserialize(Buffer.from(recipientIdentityKeyPair.publicKey));
    const recipientIdentityPrivate = signal.PrivateKey.deserialize(Buffer.from(recipientIdentityKeyPair.privateKey));
    const recipientIdentityKeyPairObj = new signal.IdentityKeyPair(recipientIdentityPublic, recipientIdentityPrivate);
