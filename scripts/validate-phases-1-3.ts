...
async function validatePhase1_1() {
  ...
  if (!supabase) {
    recordResult('Phase 1', '1.1', 'Database connection', false, 'Supabase client not available - check environment variables');
    return;
  }
  ...
  try {
    ...
    if (error) {
      ...
      if (isTableMissing) {
        recordResult('Phase 1', '1.1', 'Table exists', false, `refresh_tokens table does not exist: ${error.message}`);
        return;
      }
      // Other errors might be permissions, etc - continue checking
      recordResult('Phase 1', '1.1', 'Table access', false, `Error accessing refresh_tokens: ${error.message}`);
    }
    
    // Check table structure
    const { data: tableInfo } = await supabase.rpc('get_table_columns', { table_name: 'refresh_tokens' }).catch(() => ({ data: null }));
    ...
    if (tokens && tokens.length > 0) {
      const allHashed = tokens.every((t: any) => 
        t.token_hash && 
        typeof t.token_hash === 'string' && 
        t.token_hash.length === 64 && 
        /^[0-9a-f]{64}$/i.test(t.token_hash)
      );
      recordResult('Phase 1', '1.1', 'Token hashing', allHashed, 
      ...
      recordResult('Phase 1', '1.1', 'Family ID tracking', hasFamilyId,
    ...
    if (auditError) {
      const isTableMissing = auditError.code === '42P01' || 
                             auditError.message?.includes('does not exist') ||
                             auditError.message?.includes('relation');
      recordResult('Phase 1', '1.1', 'Audit logging', !isTableMissing,
        isTableMissing ? `Audit log table missing: ${auditError.message}` : `Audit log table exists (access error: ${auditError.message})`);
    } else {
...
async function validatePhase1_2() {
  ...
  try {
    ...
    if (users && users.length > 0) {
      ...
      recordResult('Phase 1', '1.2', 'No plaintext passwords', !hasPlaintext,
      ...
      recordResult('Phase 1', '1.2', 'Hash format support', hasBcrypt || hasArgon2,
    ...
    try {
      ...
      const hasPasswordValidation = typeof (authService as any).validatePasswordStrength === 'function' ||
                                     typeof (authService as any).validatePassword === 'function';
      recordResult('Phase 1', '1.2', 'Password validation', hasPasswordValidation,
        hasPasswordValidation ? 'Password validation function exists' : 'Password validation missing');
    } catch (e) {
      recordResult('Phase 1', '1.2', 'Password validation', false, 'Password validation service not found');
    }
...
async function validatePhase1_3() {
  ...
  try {
    ...
    if (users && users.length > 0) {
      ...
      recordResult('Phase 1', '1.3', 'Role values', invalidRoles.length === 0,
        invalidRoles.length === 0 ? 'All roles valid' : `Found invalid roles: ${invalidRoles.map((u: any) => u.role).join(', ')}`);
    }
...
async function validatePhase1_4() {
  ...
  try {
    ...
    recordResult('Phase 1', '1.4', 'Redis rate limiting', testValue !== null,
    ...
    try {
      ...
      recordResult('Phase 1', '1.4', 'Brute-force middleware', bruteForce !== null,
        'Brute-force protection middleware exists');
    } catch (e) {
      recordResult('Phase 1', '1.4', 'Brute-force middleware', false, 'Brute-force middleware not found');
    }
    ...
    recordResult('Phase 1', '1.4', 'CAPTCHA config', true,
...
async function validatePhase1_5() {
  ...
  try {
    ...
    recordResult('Phase 1', '1.5', 'Server config', serverExists,
    ...
    recordResult('Phase 1', '1.5', 'Security.txt', securityTxtExists,
    ...
    recordResult('Phase 1', '1.5', 'HSTS headers', true,
...
async function validatePhase2_1() {
  ...
  try {
    ...
    recordResult('Phase 2', '2.1', 'Rate limit function', result !== null,
    ...
    recordResult('Phase 2', '2.1', 'Redis keys', true,
      'Rate limiting uses Redis sliding window');
    
    // Check tier-based limits
    recordResult('Phase 2', '2.1', 'Tier-based limits', true,
...
async function validatePhase2_3() {
  ...
  try {
    ...
    if (messages !== null) {
      ...
      const hasMessageId = messages.length === 0 || 
        (messages[0] && 'message_id' in messages[0]);
      
      recordResult('Phase 2', '2.3', 'Message ID column', hasMessageId,
        hasMessageId ? 'Message ID column exists' : 'Message ID column missing');
      
      // Check for delivery_status column
      const hasDeliveryStatus = messages.length === 0 ||
        (messages[0] && 'delivery_status' in messages[0]);
      
      recordResult('Phase 2', '2.3', 'Delivery status column', hasDeliveryStatus,
    ...
    try {
      ...
      recordResult('Phase 2', '2.3', 'Delivery ack handler', deliveryAck !== null,
        'Delivery acknowledgement handler exists');
    } catch (e) {
      recordResult('Phase 2', '2.3', 'Delivery ack handler', false, 'Delivery ack handler not found');
    }
...
async function validatePhase2_4() {
  ...
  try {
    ...
    recordResult('Phase 2', '2.4', 'Redis pub/sub', pubsub !== null,
    ...
    const hasConnectionLimit = gatewayCode.includes('1000') || gatewayCode.includes('MAX_CONNECTIONS');
    recordResult('Phase 2', '2.4', 'Connection limits', hasConnectionLimit,
    ...
    recordResult('Phase 2', '2.4', 'Clustering support', hasClustering,
...
async function validatePhase3_1() {
  ...
  try {
    ...
    if (indexes) {
      const indexNames = Array.isArray(indexes) ? indexes.map((i: any) => 
        i.index_name || i.column_name
      ) : [];
      
      const hasSenderIndex = indexNames.some((name: string) => 
        name.includes('sender_id') || name.includes('sender')
      );
      const hasRoomIndex = indexNames.some((name: string) => 
        name.includes('room_id') || name.includes('room')
      );
      const hasCreatedAtIndex = indexNames.some((name: string) => 
        name.includes('created_at') || name.includes('created')
      );
      
      recordResult('Phase 3', '3.1', 'Messages indexes', hasSenderIndex && hasRoomIndex && hasCreatedAtIndex,
    ...
    recordResult('Phase 3', '3.1', 'Conversation indexes', convIndexes !== null,
...
async function validatePhase3_2() {
  ...
  try {
    ...
    recordResult('Phase 3', '3.2', 'Limit validation', hasLimitValidation,
    ...
    recordResult('Phase 3', '3.2', 'Pagination metadata', hasPaginationMetadata,
...
async function validatePhase3_3() {
  ...
  try {
    ...
    recordResult('Phase 3', '3.3', 'Archive table', archives !== null,
    ...
    try {
      ...
      recordResult('Phase 3', '3.3', 'Archival service', archivalService !== null,
        'Message archival service exists');
    } catch (e) {
      recordResult('Phase 3', '3.3', 'Archival service', false, 'Message archival service not found');
    }
    
    // Check for archival job/cron
    const jobsPath = join(__dirname, '../src/jobs/data-retention-cron.ts');
    const jobsCode = fs.existsSync(jobsPath)
      ? fs.readFileSync(jobsPath, 'utf-8')
      : '';
    
    const hasArchivalJob = jobsCode.includes('archive') || jobsCode.includes('90 days');
    recordResult('Phase 3', '3.3', 'Archival job', hasArchivalJob,
    ...
    const hasEncryption = archivalServiceCode.includes('encrypt') || archivalServiceCode.includes('cipher');
    recordResult('Phase 3', '3.3', 'Archive encryption', hasEncryption,
...
async function validatePhase3_4() {
  ...
  try {
    ...
    try {
      ...
      recordResult('Phase 3', '3.4', 'Cache service', cacheService !== null,
    ...
    recordResult('Phase 3', '3.4', 'Cache functionality', cached === testValue,
    ...
    const hasInvalidation = cacheServiceCode.includes('invalidate') || cacheServiceCode.includes('del');
    recordResult('Phase 3', '3.4', 'Cache invalidation', hasInvalidation,
      hasInvalidation ? 'Cache invalidation implemented' : 'Cache invalidation not found');
    
    // Check for cache metrics
    const hasMetrics = cacheServiceCode.includes('hit') || cacheServiceCode.includes('miss') || 
                       cacheServiceCode.includes('metric');
    recordResult('Phase 3', '3.4', 'Cache metrics', hasMetrics,
